...
int main()
{
    ...
    if(!Cidade.open()){
        ...
        if(Cidade.load("../csv/r1.csv",',')) cout << "Carregamento de dados efetuado com sucesso!" << endl;
    ...
    std::vector<short> tF1 {INT4, CHAR32, CHAR32};
    Relation Nome(relationNameNome, tF1);
    if(!Nome.open()){
        cout << "Carregando dados Nome para base binaria..." << endl;
        if(Nome.load("../csv/r2.csv",',')) cout << "Carregamento de dados efetuado com sucesso!" << endl;
        else { cout <<  "Falha no carregamento da base de dados." << endl; return 1; }
    }
    else cout << "Base de dados binaria pre-existente no disco foi encontrada. O carregamento e desnecessario." << endl;
