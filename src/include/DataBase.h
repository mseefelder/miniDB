...
// models a simple database system, with operations for creating and searching
// on tables
class DataBase {
...
public:
  ...
  bool createTable(std::string csvFile, char separator,
                   vector<short> tupleFormat, std::string relationName);
  ...
  bool select(std::string firstRelation, std::string *fields,
              std::string desiredField, int condition_value, int join_id = 0,
              std::string secondRelation = NULL)
};
